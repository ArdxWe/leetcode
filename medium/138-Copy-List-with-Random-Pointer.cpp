 ...
 * x.random --> y. Return the head of the copied linked list. The linked list is
 * represented in the input/output as a list of n nodes. Each node is
 * represented as a pair of [val, random_index] where: val: an integer
 * representing Node.val random_index: the index of the node (range from 0 to
 * n-1) that the random pointer points to, or null if it does not point to any
 * node. Your code will only be given the head of the original linked list.
 */
 ...
 public:
  Node* copyRandomList(Node* head) {
    ...
    for (int i = 0; i < v.size(); i++) {
      ...
      if (item->random == nullptr) {
        v[i]->random = nullptr;
      } else
        v[i]->random = v[old[item->random]];
      item = item->next;
    }
    return v[0];
  }
